...
/// Adds the standard "connection awareness" test group shared by all
/// connection-aware notifier tests.
///
/// [provider] is the provider under test (e.g. `gitReposProvider`).
/// [serviceOverrides] returns the provider overrides that inject the mock
/// client for this provider.
/// [verifyNoGrpcCalls] is a callback that asserts no gRPC methods were called,
/// e.g. `() => verifyNever(() => mockClient.listRepos(any()))`.
void connectionAwarenessTests({
  required String label,
  required dynamic provider,
  required List Function() serviceOverrides,
  ...
  group('$label - connection awareness', () {
    test('throws StateError when disconnected', () async {
      ...
      final state = dc.read(provider) as AsyncValue;
...
/// Adds the standard "error handling" test group shared by all notifier tests.
///
/// [provider] is the provider under test.
/// [errorOverrides] returns provider overrides that inject a failing client
/// for the given [GrpcError].
void errorHandlingTests({
  required String label,
  required dynamic provider,
  required List Function(GrpcError error) errorOverrides,
}) {
  group('$label - error handling', () {
    test('gRPC error is captured in state', () async {
      ...
      final state = ec.read(provider) as AsyncValue;
      expect(state.hasError, isTrue);
...
///
/// [stubListEmpty] stubs the list/build RPC to return an empty response.
/// [stubListItems] stubs the list/build RPC to return a response with items.
/// [verifyListCalled] verifies the list RPC was called exactly [n] times.
/// [resetMock] resets the mock client (e.g. `() => reset(mockClient)`).
/// [getFirstItemId] extracts an identifying field from the first item in the
/// state value, used for refresh-updates-state assertions.
class RefreshTestConfig<T> {
  ...
  /// Stubs the list RPC after reset with an empty/minimal response.
  ...
  /// Gets an identifier from the second item (for verifying refresh added it).
...
void refreshTests<T>(RefreshTestConfig<T> config) {
  group('${config.label} - refresh', () {
    test('re-fetches and updates state', () async {
      ...
      await container.read(config.provider.future);

      config.stubRefreshed();

      final notifier = container.read(config.provider.notifier);
      await (notifier as dynamic).refresh();

      final state = container.read(config.provider);
      expect(config.getItemCount(state.value as T), 2);
      expect(config.getSecondItemId(state.value as T), isNotEmpty);
    });
...
// ---------------------------------------------------------------------------
// "init then act" helper — most notifier action tests need:
//   1. stub listXxx → empty
//   2. await container.read(provider.future)
// This helper does that in one call.
// ---------------------------------------------------------------------------

/// Stubs the initial build for a list-based notifier and waits for it.
///
/// [stubEmpty] should stub the list RPC to return an empty response.
/// Returns after the provider's future resolves.
